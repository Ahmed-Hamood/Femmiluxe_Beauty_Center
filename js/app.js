...
document.addEventListener("DOMContentLoaded", () => {
 ...
 allSections = document.querySelectorAll(".section-target")
 scrollPos = document.querySelector(".main-header-container").clientHeight
...
function ActiveHeaderNav() {
 ...
 mainSection.addEventListener("scroll", () => {
  ...
  while (--len && mainSection.scrollTop + scrollPos < allSections[len].offsetTop) {}
 ...
 allNavLinks.forEach((element, index) => {
  element.addEventListener("click", () => {
   mainSection.scrollTo(0, allSections[index].offsetTop - scrollPos)
  })
 })

 // View service scroll to its section
 document.querySelector(".view-services-btn").addEventListener("click", e => {
  mainSection.scrollTo(0, allSections[2].offsetTop - scrollPos)
 })
}
 ...
 all_service_type_link.forEach(element => {
  element.addEventListener("click", el1 => {
   ...
   if (element.getAttribute("name") == "hairCare") {
    ...
    mainSection.scrollTo(0, service_hairCare_desc.offsetTop - scrollPos)
   }
   if (element.getAttribute("name") == "nailCare") {
    service_nailCare_desc.style["display"] = "flex"
    mainSection.scrollTo(0, service_nailCare_desc.offsetTop - scrollPos)
   }
   if (element.getAttribute("name") == "bodyCare") {
    service_bodyCare_desc.style["display"] = "flex"
    mainSection.scrollTo(0, service_bodyCare_desc.offsetTop - scrollPos)
   }
   if (element.getAttribute("name") == "faceCare") {
    service_faceCare_desc.style["display"] = "flex"
    mainSection.scrollTo(0, service_faceCare_desc.offsetTop - scrollPos)
   }
  })
 })
}
